...
module.exports = {
  ...
  extractor: (keyValues, rawText) => {
    ...
    const first4Lines = rawText.slice(0, 4);
    const fullName = first4Lines.find((line) => (line !== 'aps'
      && line !== 'Your electricity bill'
      && !line.match(/Bill date/)
    ));
    let first_name;
    let last_name;
    if (fullName) {
    ...
    // account number
    let account_number = keyValues['Your account number'];
    if (!account_number || !account_number.length) {
      account_number = keyValues['YOUR ACCOUNT NUMBER:'];

      // try on second position
      if (!account_number || !account_number.length) {
        const accountNumberIndex = rawText.findIndex((line) => line === 'Your account number');
        account_number = rawText.slice(accountNumberIndex, 7).find((line) => line.match(/^\d+$/g));
      }
    }
    if (!account_number.match(/^\d+$/g)) {
      account_number = '';
    }
