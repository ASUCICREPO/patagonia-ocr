...
const fetchAllPages = async (jobId, requestId) => {
  const pages = [];
  const params = {
    JobId: jobId,
  };
...
module.exports.storeOutput = async (jobId, requestId) => {
  console.log('storeOutput', jobId, requestId);
  // fetch output from Textract API
  const output = await fetchAllPages(jobId, requestId);
