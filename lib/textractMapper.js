...
module.exports = (textractOutputBlocks) => {
  const mapped = {
    keyValues: {},
    rawText: [],
  };

  // sum all LINE blocks as rawText
  textractOutputBlocks
    .filter((block) => block.BlockType === 'LINE')
    .forEach((block) => mapped.rawText.push(`${block.Text}`));

  // generate detected key-value pairs
  const { keys, values, blocks } = getMap(textractOutputBlocks);
