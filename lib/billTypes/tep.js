module.exports = {
  ...
  extractor: (keyValues, rawText) => {
    ...
    let serviceNumber = /(Service No\.)\ (\d+)/gm.exec(string);

    normalized['service_number'] = serviceNumber[2].trim();

    // account number
    normalized['account_number'] = [...new Set(keyValues.filter(kv => {
      return kv.key.match(/Account/g) && kv.value.match(/\d+/g);
    }))][0].value.trim();

    // full address found in line before service number
    let regex = `Service No. ${normalized['service_number']}*`;
    normalized['address'] = array[array.findIndex(line => !!line.match(regex)) - 1];

    // bill amount (used for finding name too)
    normalized['bill_amount'] = keyValues.filter(kv => {
      return kv.key.match(/Total Amount Due/g) && kv.value.match(/\$\d+/g);
    })[0].value.trim();

    // bill date
    normalized['bill_date'] = keyValues.filter(kv => {
      return kv.key.match(/Bill date/g) && kv.value.match(/\$\d+/g);
    })[0].value.trim();
