...
it('Expect APS bill to be detected and processed', async () => {
	const response = inputFromFile(`${__dirname}/../_samples/textract_output_aps_bill.json`);
  ...
  expect(response.extracted.bill_date).toBe('May 8, 2020');
  expect(response.extracted.account_number).toBe('4563211000');
});

it('Expect SOUTHWESTGAS bill to be detected and processed', async () => {
	const response = inputFromFile(`${__dirname}/../_samples/textract_output_southwest_gas_bill.json`);
  ...
  expect(response.extracted.bill_date).toBe('05/01/20');
...
it('Expect CITYOFPHOENIXWATER bill to be detected and processed', async () => {
	const response = inputFromFile(`${__dirname}/../_samples/textract_output_city_of_phoenix_water_bill.json`);
...
it('Expect SRP bill to be detected and processed', async () => {
	const response = inputFromFile(`${__dirname}/../_samples/textract_output_srp_bill.json`);
  ...
  expect(response.extracted.bill_date).toBe('5/12/2020');
